...
class WebhookRepository(BaseRepository[WebhookEndpoint]):
    ...
    async def get_active_endpoints(self, merchant_id: UUID) -> list[WebhookEndpoint]:
    ...
    async def get_endpoints_for_event(self, merchant_id: UUID, event_type: str) -> list[WebhookEndpoint]:
        endpoints = await self.get_active_endpoints(merchant_id)
